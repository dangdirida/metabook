...
export async function POST(req: NextRequest) {
  ...
  if (type === "perspective") {
    systemPrompt = `당신은 뛰어난 한국 소설 작가입니다. 주어진 원문을 특정 인물의 시점에서 문학적으로 재집필합니다. 반드시 한국어로 작성하고, ## 챕터 제목 형식으로 2~3개 챕터로 나누어 작성하세요.`;
...
${character}의 내면 심리와 감정을 중심으로, 원문의 핵심 사건을 유지하면서 완전히 새로운 시점으로 재집필해주세요. 대화문과 내면 독백을 풍부하게 포함하세요.`;
    ...
    systemPrompt = `당신은 뛰어난 한국 소설 작가입니다. 주어진 원문의 분기점 이후를 새로운 방향으로 재집필합니다. 반드시 한국어로 작성하고, ## 챕터 제목 형식으로 3개 챕터로 나누어 작성하세요.`;
    userPrompt = `원작: "${bookTitle}" (저자: ${bookAuthor}, ${chapterRange})
