...
import {
  X, BookOpen, Film, Gift, Heart, Sparkles, ChevronRight, ChevronUp,
  RefreshCw, Bookmark, Smile, Image as ImageIcon, GripHorizontal, Play, Plus,
} from "lucide-react";
    ...
    <div className="bg-white dark:bg-mono-900 border-b border-mono-200 dark:border-mono-800 flex flex-col flex-shrink-0">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          {creations.length > 0 && (
            <span className="text-[10px] font-semibold bg-primary-500 text-white px-1.5 py-0.5 rounded-full">{creations.length}</span>
          )}
        </div>
      ...
      {galleryOpen && (
        <div className="flex justify-center py-1 cursor-ns-resize hover:bg-mono-50 select-none"
          onMouseDown={(e) => {
            e.preventDefault();
            const startY = e.clientY; const startH = galleryHeight;
            const onMove = (ev: MouseEvent) => setGalleryHeight(Math.min(560, Math.max(140, startH + ev.clientY - startY)));
      ...
      <div className={`overflow-hidden transition-all duration-300 ${galleryOpen ? "" : "max-h-0"}`} style={galleryOpen ? { maxHeight: galleryHeight } : undefined}>
        <div className="flex flex-wrap gap-2 justify-start px-4 py-2.5 border-b border-mono-100 dark:border-mono-800">
          {(["shortbook", "shortmovie", "goods"] as const).map((type) => {
            ...
            const Icon = cfg.icon;
            return (
              <button key={type} onClick={() => setOpenModal(type)}
        ...
        <div className="overflow-y-auto custom-scrollbar p-3" style={{ maxHeight: "calc(100% - 56px)" }}>
          {creations.length === 0 ? (
            <div className="text-center py-6">
              ...
              <p className="text-xs text-mono-400 mt-1 leading-relaxed">AI로 숏북, 뮤비, 굿즈를<br />낙닭 만들 수 있어요 ✨</p>
              <button onClick={() => setOpenModal("shortbook")}
            ...
            <div className="grid grid-cols-5 gap-2">
              {creations.map((item) => {
                const cfg = TYPE_CONFIG[item.type]; const Icon = cfg.icon;
                return <CreationCard key={item.id} item={item} cfg={cfg} Icon={Icon} onOpen={() => setOpenModal(item.type as ModalType)} onHeart={() => handleHeart(item.id)} />;
              })}
            </div>
          )}
        </div>
      ...
      {openModal && (
        <IntroModal type={openModal} bookId={bookId as string} onClose={() => setOpenModal(null)}
...
function CreationCard({ item, cfg, Icon, onOpen, onHeart }: {
  item: CreationItem; cfg: { label: string; gradient: string; accentColor: string; icon: React.ElementType };
  Icon: React.ElementType; onOpen: () => void; onHeart: () => void;
}) {
  return (
...
function IntroModal({ type, bookId, onClose, onNavigate }: {
  type: "shortbook" | "shortmovie" | "goods"; bookId: string; onClose: () => void; onNavigate: (path: string) => void;
}) {
  const configs = {
    shortbook: {
      title: "숏북 만들기", emoji: "📖",
      desc: "책의 특정 구간을 AI가 완전히 새롭게 재집필해요.
다른 인물의 시점으로 보거나, 결말을 바꾼볼 수 있어요.",
      features: [
        { title: "다른 시점으로", desc: "선택한 인물의 눈으로 이야기를 다시 써요" },
        { title: "다른 결말으로", desc: "내가 원하는 방향으로 결말을 바꾽요" },
      ],
      ctaLabel: "숏북 만들러 가기", ctaClass: "bg-[var(--color-primary-500)] hover:bg-[var(--color-primary-600)]",
...
원하는 구간과 인물을 선택하면 5~8초 영상이 완성돼요.",
      features: [{ title: "장면 영상화", desc: "텍스트 장면을 720p HD 영상으로 변환해요" }],
      ctaLabel: "숏뮤비 만들러 가기", ctaClass: "bg-[var(--color-secondary-500)] hover:bg-[var(--color-secondary-600)]",
    ...
    goods: {
      ...
      ctaLabel: "굿즈 만들러 가기", ctaClass: "hover:opacity-90",
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md flex flex-col">
        ...
        <div className="p-5 space-y-4">
          ...
          <button onClick={() => onNavigate(config.path)}
