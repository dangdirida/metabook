...
export default function UserMenu() {
  ...
  return (
    <div className="flex items-center gap-2">
      <Link href="/favorites" className="p-2 rounded-xl hover:bg-[var(--color-mono-050)] transition-colors" title="찜한 도서">
      ...
      <div className="relative" ref={menuRef}>
        <button onClick={() => setOpen(!open)}
          ...
          aria-label="사용자 메뉴">
          {user.image ? (
            <Image src={user.image} alt={user.name || "User"} width={32} height={32} className="w-full h-full object-cover" unoptimized />
          ) : (
            <div className="w-full h-full bg-primary-100 flex items-center justify-center">
              <span className="text-sm text-primary-600 font-medium">{initial}</span>
            </div>
          )}
        </button>
        {open && (
          <div className="absolute right-0 top-10 w-56 bg-white rounded-xl border border-mono-200 shadow-lg z-50 py-2">
            <div className="px-4 py-3 border-b border-mono-100">
              <p className="text-sm font-medium text-mono-900 truncate">{user.name}</p>
